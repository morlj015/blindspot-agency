...
// Config
const WEBHOOK_URL = 'https://hook.eu1.make.com/i4jhn2699jgmibw6jxub322k7496i5sd';
...
async function submitForm(formEl, successMsg, modalId) {
  ...
  if (firstInvalid) {
    const label = formEl.querySelector(`label[for="${firstInvalid.id}"]`);
    const fieldName = label?.textContent?.trim() || 'Please complete all required fields.';
    if (errorEl) {
  ...
  data.formType = formEl.id === 'form-quote' ? 'quote_request' : 'book_call';
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Webhook failed with status ${response.status}: ${message}`);
    }
